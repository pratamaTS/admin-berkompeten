...
useRouter
  ...
  const fetchData = async (id) => {
    try {
      ...
      `https://gateway.berkompeten.id/api/admin/master/educational-status/detail?id=${id}`, {
        headers: {
      ...
      console.error("Error fetching educational status data:", error);
    }
  };

  const handleSubmit = async () => {
    try {
      const url = `https://gateway.berkompeten.id/api/admin/master/educational-status/upsert`;
      const method = 'post';
      ...
      successMessage.value = 'Educational Status saved successfully!';
      setTimeout(() => {
        resetForm()
        router.push('/educational-status-management'); // Redirect to the questions list page after a delay
      }, 2000);
    } catch (error) {
...
<template>
  <VRow>
    <VCol cols="12">
      <VCard title="Educationa Status Details">
        <VCardText>
