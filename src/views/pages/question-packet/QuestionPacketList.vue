...
const fetchData = async (page = 1, searchQuery = '') => {
  try {
    let url = `https://gateway.berkompeten.id/api/admin/master/question-packet?page=${page}`;
    if (searchQuery) {
...
const deleteData = async (id) => {
  try {
    const response = await axios.delete(
      `https://gateway.berkompeten.id/api/admin/master/question-packet?question_packet_id=${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
