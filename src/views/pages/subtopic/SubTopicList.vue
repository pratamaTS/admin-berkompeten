...
const fetchData = async (page = 1, searchQuery = '') => {
  try {
    let url = `https://gateway.berkompeten.id/api/admin/master/subtopic?page=${page}`;
    if (searchQuery) {
...
const deleteData = async (id) => {
  try {
    const response = await axios.delete(`https://gateway.berkompeten.id/api/admin/master/subtopic?id=${id}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
...
<template>
  <div>
    <VCardTitle class="mb-4">Sub Topic Management</VCardTitle>
