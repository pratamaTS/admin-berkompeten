...
const fetchData = async (page = 1, searchQuery = '') => {
  try {
    let url = `https://gateway.berkompeten.id/api/admin/master/university?page=${page}`;
    if (searchQuery) {
...
const deleteData = async (id) => {
  try {
    const response = await axios.delete(`https://gateway.berkompeten.id/api/admin/master/university?id=${id}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
