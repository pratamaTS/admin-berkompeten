...
const fetchData = async (page = 1, searchQuery = '') => {
  try {
    let url = `https://gateway.berkompeten.com/api/admin/master/membership?page=${page}`;
    if (searchQuery) {
...
const deleteData = async (id) => {
  try {
    await axios.delete(`https://gateway.berkompeten.com/api/admin/master/membership?id=${id}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
