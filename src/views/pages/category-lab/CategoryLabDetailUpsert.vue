...
useRouter
  ...
  const fetchData = async (id) => {
    try {
      ...
      `https://gateway.berkompeten.id/api/admin/master/category-lab/detail?id=${id}`, {
        headers: {
      ...
      console.error("Error fetching category lab data:", error);
    }
  };

  const handleSubmit = async () => {
    try {
      const url = `https://gateway.berkompeten.id/api/admin/master/category-lab/upsert`;
      const method = 'post';
      ...
      successMessage.value = 'Category lab saved successfully!';
      setTimeout(() => {
        resetForm()
        router.push('/category-lab-management'); // Redirect to the questions list page after a delay
      }, 2000);
    } catch (error) {
...
<template>
  <VRow>
    <VCol cols="12">
      <VCard title="Category Lab Details">
        <VCardText>
